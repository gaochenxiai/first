...
{ 
	if (NewState!=DISABLE)printf("usb使能上拉\r\n");//usb pull up enable
	else printf("usb失能上拉\r\n"); //usb pull up disable
}
...
bool USART_Config(void)
{
	...
	printf("linecoding.format校验位:%d\r\n",linecoding.format);
	printf("linecoding.paritytype停止位:%d\r\n",linecoding.paritytype);
	printf("linecoding.datatype数据位:%d\r\n",linecoding.datatype);
	printf("linecoding.bitrate波特率:%d\r\n",linecoding.bitrate);
	return (TRUE);
}
